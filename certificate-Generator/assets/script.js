...
downloadButton.addEventListener("click", function () {
  ...
  if (downloadType == "png" || downloadType == "jpg") {
    ...
    pdf.addImage(canvas.toDataURL("image/png"), "PNG", 8, 8,280,195);
    pdf.save("certificate.pdf");
    // using html2canvas and iframe concept of #certificatecanvas
    // create the image url
    // var image = canvas.toDataURL("image/png");
    // pdf.addImage(imgData, 'PNG', 8, 8, 280, 195);
    // create the iframe of image
...
downloadZipButton.addEventListener("click", function (e) {
  ...
  sheetData.forEach(function (row, i) {
    ...
    var filename = row[dataIndex[i]] + ".png";
...
//  zippdfdowload button click event listener to download zip file of pdfs of certificates generated from the excel sheet
downloadPdfZipButton.addEventListener("click", function (e) {
  // Start recording Time
  var startTime = new Date();
  ...
  sheetData.forEach(function (row, i) {
    ...
    // add the image in landscape mode in the pdf
    pdf.addImage(imgData, 'PNG', 8, 8, 280, 195);
    // pdf.save("Cerrt_" + (i + 1) + ".pdf");
    ...
    // loading a file and add it in a zip file
    zip.file(row[dataIndex[i]] + ".pdf", pdf.output('blob'), { binary: true });
  });
  zip.generateAsync({type:"blob"}).then(function(content) {
